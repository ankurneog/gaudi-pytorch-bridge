...
#include "jit_fork/frontend/schema_matching.h"
...
std::ostream& Graph::print(std::ostream& out, bool print_source_info) const {
  ...
  auto checkValue = [&](const Value* v) {
    if (TensorTypePtr type = v->type()->cast<TensorType>()) {
      if (type->device() && !has_device) {
        ...
        HABANA_ASSERT(device == type->device());
      }
    }
  };
...
void Node::lint() const {
  ...
  switch (kind()) {
    case prim::Constant:
      HABANA_ASSERT(inputs_.empty());
      break;
    case prim::Return:
      // Return uses is zero
      HABANA_ASSERT(outputs().empty());
      break;
    case prim::Param:
...
void Graph::lint() const {
  ...
  struct LintImpl {
    ...
    void check_block(const Block* b) {
      ...
      HABANA_ASSERT(b->output_->kind() == prim::Return);
      ...
      HABANA_ASSERT(std::includes(ALL_OF(all_nodes_set), ALL_OF(nodes_set)));
      HABANA_ASSERT(std::includes(ALL_OF(all_nodes_set), ALL_OF(inputs_set)));
      HABANA_ASSERT(std::includes(ALL_OF(all_nodes_set), ALL_OF(output_set)));
...
bool Value::isValidName(const std::string& name, bool allow_numbers) {
  ...
  if (isNumber(name) && !allow_numbers) {
    return false;
  }
...
Value* Value::setDebugName(const std::string& name, bool allow_numbers) {
  if (!isValidName(name, allow_numbers)) {
  ...
  if (old_owner_of_name != names.end()) {
    ...
    do {
      std::stringstream ss;
#ifndef _WIN32
      // Protect 12345 integer from becoming "1,2345" if some other process
      // sets global locale For more details see
...
void Value::replaceFirstUseWith(Value* newValue) {
  HABANA_ASSERT(owningGraph() == newValue->owningGraph());
  auto u = uses()[0];
...
bool Node::matches(const FunctionSchema& schema) const {
  ...
  for (const auto i : c10::irange(formals.size())) {
    ...
    // not resolved all type variables, e.g. if None was matched to
...
bool Node::hasSideEffects() const {
  ...
  if (kind_.is_prim() || kind_.is_aten() || kind_.is_cuda()) {
    ...
    // AliasAnalysisKind::CONSERVATIVE but this is the intended behavior for
    // all current ops and a good error check. We can consider lifting this
    // constraint later if we have a use case for it.

    HABANA_ASSERT(
        (op->aliasAnalysisKind() ==
             c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE ||
         op->aliasAnalysisKind() == c10::AliasAnalysisKind::FROM_SCHEMA ||
         op->aliasAnalysisKind() == c10::AliasAnalysisKind::CONSERVATIVE),
        "aten:: and prim:: ops should have c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE, c10::AliasAnalysisKind::FROM_SCHEMA or c10::AliasAnalysisKind::CONSERVATIVE but",
        kind_.toDisplayString(),
        " has ",
  ...
  HABANA_ASSERT(false, "Unhandled c10::AliasAnalysisKind case");
...
// case (appending to the end of the graph) is made more efficient by
// advancing a fixed interval past the previous node and placing `this` there.
// Otherwise, assign `this` a position at the midpoint between its prev() and
// next() nodes.
//
...
void Node::assignTopoPosition() {
  ...
  if (is_last) {
    ...
    HABANA_ASSERT(prevPos < topo_position_ && topo_position_ < nextPos);
  }
}
...
void Node::eraseOutput(size_t i) {
  HABANA_ASSERT(i < outputs_.size());
  HABANA_ASSERT(outputs_[i]->uses().empty());
  op_ = nullptr;
  Value* n = outputs_[i];
...
void Node::replaceAllUsesWith(Node* n) {
  HABANA_ASSERT(outputs().size() == n->outputs().size());
...
Value* Node::insertInput(size_t i, Value* value) {
  HABANA_ASSERT(graph_ == value->owningGraph());
  op_ = nullptr;
...
Value* Node::replaceInput(size_t i, Value* newValue) {
  HABANA_ASSERT(newValue->owningGraph() == graph_);
  op_ = nullptr;
...
void Node::replaceInputWith(Value* from, Value* to) {
  HABANA_ASSERT(from->owningGraph() == graph_);
...
bool Node::isBeforeOrAfter(const Node* n, MoveSide moveSide) const {
  ...
  // should never reach here, since both nodes are ultimately in the same
  // graph
  HABANA_ASSERT(false);
}
...
Node* Node::insertBefore(Node* n) {
  HABANA_ASSERT(n->inBlockList());
  insertAfter(n->prev());
  return this;
}

Node* Node::insertAfter(Node* n) {
  HABANA_ASSERT(!inBlockList() && n->inBlockList());
  HABANA_ASSERT(n->owningBlock());
  HABANA_ASSERT(
      n->kind() != prim::Return,
...
void Node::permuteInputs(const std::vector<size_t>& new_order) {
  op_ = nullptr;
  HABANA_ASSERT(new_order.size() == inputs_.size());
  std::vector<Value*> new_inputs;
  new_inputs.reserve(new_order.size());
  for (const auto i : c10::irange(new_order.size())) {
    HABANA_ASSERT(inputs_.at(new_order[i]) != nullptr, "Repeated index");
...
void Node::permuteOutputs(const std::vector<size_t>& new_order) {
  op_ = nullptr;
  HABANA_ASSERT(new_order.size() == outputs_.size());
  std::vector<Value*> new_outputs;
  new_outputs.reserve(new_order.size());
  for (const auto i : c10::irange(new_order.size())) {
    HABANA_ASSERT(outputs_.at(new_order[i]) != nullptr, "Repeated index");
...
use_list::iterator Node::findUseForInput(size_t i) {
  ...
  HABANA_ASSERT(use_it != input_uses.end());
  return use_it;
}
...
Block* Node::findCommonAncestorBlockWith(Node* n) {
  ...
  while (true) {
    ...
    n1 = n1->owningBlock()->owningNode();
    n2 = n2->owningBlock()->owningNode();
    HABANA_ASSERT(n1 != nullptr);
    HABANA_ASSERT(n2 != nullptr);
  }
}
...
Node* Graph::createUninitialized(TypePtr type) {
...
std::string Graph::toString(bool print_source_info) const {
  // GAUDI_JIT_TRACE;
  std::ostringstream oss;
  print(oss, print_source_info);
  return oss.str();
}
...
void Node::copyAttributesIntoUpstreamNode(::torch::jit::Node* dst_node) {
  const auto& src_node = *this;

  for (const AVPtr& src_attr : src_node.values_) {
    ::c10::Symbol src_attr_name = src_attr->name;
    ...
    switch (type) {
      ...
      case AttributeKind ::fs: {
        auto gotAttr = this->getAttr<FloatsAttr>(src_attr_name);
        dst_node->fs_(src_attr_name, gotAttr);
        break;
      }
      case AttributeKind ::c: {
        auto gotAttr = this->getAttr<ComplexAttr>(src_attr_name);
        dst_node->c_(src_attr_name, gotAttr);
        break;
      }
      case AttributeKind ::cs: {
        auto gotAttr = this->getAttr<ComplexValsAttr>(src_attr_name);
        dst_node->cs_(src_attr_name, gotAttr);

        break;
      }
      case AttributeKind::i: {
        auto gotAttr = this->getAttr<IntAttr>(src_attr_name);
        dst_node->i_(src_attr_name, gotAttr);
        break;
      }
      case AttributeKind ::is: {
        auto gotAttr = this->getAttr<IntsAttr>(src_attr_name);
        dst_node->is_(src_attr_name, gotAttr);

        break;
      }
      case AttributeKind ::s: {
        auto gotAttr = this->getAttr<StringAttr>(src_attr_name);
        dst_node->s_(src_attr_name, gotAttr);

        break;
      }
      case AttributeKind ::ss: {
        auto gotAttr = this->getAttr<StringsAttr>(src_attr_name);
      ...
      case AttributeKind ::ival: {
        auto gotAttr = this->getAttr<IValueAttr>(src_attr_name);
...
at::ArrayRef<Value*> createTupleUnpack(Value* v) {
  // small peephole optimization to ensure IntArrayRef attributes can still
  // turn into constants e.g. in x.expand([3, 4])
...
std::vector<Value*> insertGraph(
  ...
  HABANA_ASSERT(callee.inputs().size() == inputs.size());
  for (const auto i : c10::irange(inputs.size())) {
...
std::vector<std::shared_ptr<torch::jit::Operator>> OperatorSet::getOps() const {
  std::vector<std::shared_ptr<torch::jit::Operator>> result;
