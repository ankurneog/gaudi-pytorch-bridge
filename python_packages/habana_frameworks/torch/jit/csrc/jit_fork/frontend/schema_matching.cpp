...
static Value* emitBuiltinNode(
  ...
  return graph.packValues(n->outputs(), matched_schema.return_field_names);
}
...
Value* emitBuiltinCall(
    ...
    at::ArrayRef<NamedValue> kwargs,
    const std::optional<NamedValue>& self) {
  const auto& variants = torch::jit::getAllOperatorsFor(name);
  const auto& builtin_functions = torch::jit::getAllBuiltinFunctionsFor(name);
  ...
  for (const std::shared_ptr<torch::jit::Operator>& op : variants) {
  ...
  if (schemas.empty()) {
    ...
    if (close_symbols.empty()) {
      ...
      error_msg << "Here are some suggestions: \n";
      for (const auto& sym : close_symbols) {
        error_msg << "\t" << sym.toQualString() << "\n";
      }
  ...
  if (matched.first < variants.size() + upgrader_schemas.size()) {
    ...
    /*
      TODO: implement Function support and
        ...
        This is the case when node from builtin_functions can be added.
        This node can be inserted as a subgraph to a graph.

        Original solution:

          Finding builtin_functions:
          auto& fn = *builtin_functions[matched.first - variants.size()];

          Casting builtin_functions to GraphFunction:
          auto subgraph = *toGraphFunction(fn).graph()
