...
TEST_F(HpuOpTest, _upsample_nearest_exact1d_fwd_scale) {
...
TEST_F(HpuOpTest, _upsample_nearest_exact1d_fwd_size) {
  GenerateInputs(1, {{2, 3, 3}});
  std::vector<int64_t> size = {6};

  auto expected = torch::_upsample_nearest_exact1d(GetCpuInput(0), size);
  auto result = torch::_upsample_nearest_exact1d(GetHpuInput(0), size);

  Compare(expected, result);
}

TEST_F(HpuOpTest, _upsample_nearest_exact1d_fwd_out) {
  GenerateInputs(1, {{5, 6, 7}});
...
TEST_F(HpuOpTest, _upsample_nearest_exact1d_fwd_usual) {
  GenerateInputs(1, {{4, 3, 25}});
  std::vector<int64_t> size = {4};
  std::optional<double> scales = 3.0;

  auto expected =
      torch::_upsample_nearest_exact1d(GetCpuInput(0), size, scales);
  auto result = torch::_upsample_nearest_exact1d(GetHpuInput(0), size, scales);
  Compare(expected, result);
}

TEST_F(HpuOpTest, _upsample_nearest_exact1d_bwd_size) {
  GenerateInputs(1, {{1, 4, 6}});
  std::vector<int64_t> out_size = {6};
  std::vector<int64_t> input_size = {1, 4, 3};

  auto expected = torch::_upsample_nearest_exact1d_backward(
      GetCpuInput(0), out_size, input_size);
  auto result = torch::_upsample_nearest_exact1d_backward(
      GetHpuInput(0), out_size, input_size);
  Compare(expected, result);
}

TEST_F(HpuOpTest, _upsample_nearest_exact1d_bwd_scale) {
...
TEST_F(HpuOpTest, _upsample_nearest_exact1d_bwd_out) {
...
TEST_F(HpuOpTest, _upsample_nearest_exact1d_bwd_usual) {
  GenerateInputs(1, {{1, 4, 6}});
  std::vector<int64_t> out_size = {6};
  ...
  torch::_upsample_nearest_exact1d_backward(
